...
export default function groupBy(collection, it) {
  ...
  for (let item of collection) {
    const transformedItem = isFunction ? it(item) : item[it];
    const newValue = result?.[transformedItem]?.length
      ? [...result[transformedItem], item]
      : [item];
    result[transformedItem] = newValue;
  }
  return result;
}
