...
export default function learn(maxTime, courses) {
  ...
  for (let i = 0; i < courses.length-1; i++) {
    for (let j = i + 1; j < courses.length; j++) {
      const currentTimeCount = courses[i] + courses[j];
      if (currentTimeCount <= maxTime && currentTimeCount > coursesTime) {
        coursesTime = currentTimeCount;
        rightCourses[0] = i;
        rightCourses[1] = j;
      }
    }
  }
  return rightCourses.length ? rightCourses : null;
}
