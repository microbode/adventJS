...
export default function countPackages(carriers, carrierID) {
  ...
  const countCarrierPackages = carrierID => {
    const carrier = carriers.find(carrier => carrier[0] === carrierID);
    const carrierPackages = carrier[1];
    const carrierSubs = carrier[2];
    totalPackages += carrierPackages;
    if (carrierSubs.length) {
      carrierSubs.forEach(sub => countCarrierPackages(sub));
    }
  };

  countCarrierPackages(carrierID);
