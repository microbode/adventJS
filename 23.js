...
export default function canReconfigure(from, to) {
  const alreadyExistsFrom = (dictionary, charFrom) => {
    const values = Object.keys(dictionary);
    return values.includes(charFrom);
  };

  const alreadyExistsTo = (dictionary, charTo) => {
    const values = Object.values(dictionary);
    return values.includes(charTo);
  };
  ...
  let result = true;
  fromArray.forEach((charFrom, index) => {
    const charTo = toArray[index];
    if (
      alreadyExistsFrom(dictionary, charFrom) ||
      alreadyExistsTo(dictionary, charTo)
    ) {
      if (dictionary[charFrom] !== charTo) {
        result = false;
        return;
      }
    } else {
      dictionary[charFrom] = charTo;
    }
  });
  return result;
}
